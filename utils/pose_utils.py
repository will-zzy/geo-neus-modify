...
def save_poses(basedir, poses, pts3d, perm):
    pts_arr = []
    vis_arr = []
    # pts_view_id = [[] for view in range(0,poses.shape[-1])]
    pts_view_id = np.zeros([poses.shape[-1],len(pts3d)],dtype=bool)
    for idx,k in enumerate(pts3d):
        pts_arr.append(pts3d[k].xyz)
        for ind in pts3d[k].image_ids:
            pts_view_id[ind-1,idx] = True
    # for i in range(0,poses.shape[-1]):
    #     for j in pts3d:
    #         print(pts3d[j].image_ids)
    ...
    pts = np.stack(pts_arr, axis=0)#[n,3]
    ...
    os.makedirs(os.path.join(basedir,"sfm_pts"),exist_ok=True)
    np.save(os.path.join(basedir,'sfm_pts/points.npy'),pts)
    
    np.save(os.path.join(basedir,'sfm_pts/view_id.npy'),pts_view_id)
