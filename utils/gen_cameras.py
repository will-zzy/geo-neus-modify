...
if __name__ == '__main__':
    ...
    scale_mat = np.diag([radius, radius, radius, 1.0]).astype(np.float32)#绝对长度
    ...
    for i, image_path in enumerate(image_list):
        ...
        cv.imwrite(os.path.join(out_dir, 'images', '{:0>3d}.png'.format(i)), img)
        cv.imwrite(os.path.join(out_dir, 'masks', '{:0>3d}.png'.format(i)), np.ones_like(img) * 255)
